...
async function ensureServerServiceAuth() {
  ...
  const serviceEmail = process.env.SERVER_AUTH_EMAIL || process.env.MIGRATION_ADMIN_EMAIL || 'yannick.ekani_test@activa.local';
  const servicePass = process.env.SERVER_AUTH_PASSWORD || process.env.MIGRATION_ADMIN_PASSWORD;
  if (!servicePass) {
    return null;
  }
  try {
    const cred = await signInWithEmailAndPassword(auth, serviceEmail, servicePass);
    return cred.user;
  } catch (err: any) {
