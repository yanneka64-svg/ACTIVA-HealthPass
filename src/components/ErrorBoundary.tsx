...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.error) {
      // === AMÉLIORATION AJOUTÉE : cohérence des couleurs (audit design, 2026-09-11) — le bleu
      // marine "accent" utilisait ici #0A347B/#0D2B63, deux valeurs proches mais distinctes du
      // token officiel `brand-900` (#0a2e6b, roleTheme.ts) déjà utilisé pour ce même rôle
      // sémantique ailleurs dans l'app. Unifié sur #0a2e6b — même teinte que produisait déjà le
      // token, aucun changement visuel perceptible.
      return (
        <div className="min-h-screen w-full flex items-center justify-center bg-[#F8FAFC] p-6">
          <div className="max-w-sm w-full bg-white rounded-2xl border border-[#E8EDF2] shadow-lg p-8 text-center">
            ...
            <h1 className="mt-5 text-lg font-black text-[#0a2e6b]">Something went wrong</h1>
            ...
            <button
              ...
              className="mt-6 w-full py-2.5 px-4 rounded-xl bg-[#0a2e6b] hover:bg-[#072659] text-white text-sm font-bold shadow-sm transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer"
            >
