...
export const MEDICAL_FIELD_ENCRYPTION_KEY = defineSecret('MEDICAL_FIELD_ENCRYPTION_KEY');
...
function getKeyBuffer(): Buffer {
  const raw = MEDICAL_FIELD_ENCRYPTION_KEY.value();
  if (!raw) {
    throw new Error(
      'MEDICAL_FIELD_ENCRYPTION_KEY secret is not configured. Run: firebase functions:secrets:set MEDICAL_FIELD_ENCRYPTION_KEY'
    );
  }
...
export function encryptField(plaintext: string | undefined | null): string | undefined | null {
  ...
  return ENCRYPTED_FIELD_PREFIX + Buffer.concat([iv, authTag, encrypted]).toString('base64');
}
 ...
 */
export function decryptField(value: string | undefined | null): string | undefined | null {
  if (!value || !value.startsWith(ENCRYPTED_FIELD_PREFIX)) return value;
  const key = getKeyBuffer();
  const raw = Buffer.from(value.slice(ENCRYPTED_FIELD_PREFIX.length), 'base64');
...
export function encryptFieldMap(fields: Record<string, unknown>): Record<string, unknown> {
...
export function decryptFieldMap(fields: Record<string, unknown>): Record<string, unknown> {
  const keys = Object.keys(fields);
  if (keys.length > MAX_FIELDS_PER_CALL) {
    throw new Error(`Too many fields in a single call (max ${MAX_FIELDS_PER_CALL}).`);
  }
