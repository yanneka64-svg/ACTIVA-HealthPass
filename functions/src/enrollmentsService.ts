...
import { generateNextCardNumberServer } from './cardService';
...
export async function syncApprovedEnrollmentToMembersServer(
  db: admin.firestore.Firestore,
  enrollment: admin.firestore.DocumentData,
  approverId: string
): Promise<string | undefined> {
  const isPrincipal =
    enrollment.relationship === 'Principal' ||
  ...
  let primaryDoc: admin.firestore.QueryDocumentSnapshot | undefined;
  if (enrollment.mainInsuredCardNo) {
    ...
    if (!byCardNo.empty) primaryDoc = byCardNo.docs[0];
  }
  if (!primaryDoc && enrollment.mainInsuredName) {
    ...
    if (!byName.empty) primaryDoc = byName.docs[0];
  }

  const relType =
    enrollment.relationship === 'Conjoint' || enrollment.relationship === 'Spouse'
      ? 'spouse'
  ...
  if (primaryDoc) {
    ...
    const currentDeps: any[] = Array.isArray(primary.dependents) ? [...primary.dependents] : [];
    const existingIdx = currentDeps.findIndex(
      (d) => d.cardNo === enrollment.cardNo || String(d.fullName || '').toLowerCase().trim() === String(enrollment.fullName || '').toLowerCase().trim()
    );
    if (existingIdx >= 0) {
  ...
  const primaryCardNo =
    enrollment.mainInsuredCardNo ||
    (await generateNextCardNumberServer(db, {
...
export async function processEnrollmentDecisionServer(
  db: admin.firestore.Firestore,
  ...
  let enrollmentForSync: admin.firestore.DocumentData | undefined;

  const txResult = await db.runTransaction(async (tx: admin.firestore.Transaction) => {
